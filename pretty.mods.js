...
{
    chart_counter++;
    if ( chart_counter % chart_update_interval > 0 ) return false;
    chart_data.push(
    {
        x: chart_counter,
        y: mass / 100
...
{
    my_color = color;
    if ( chart === null ) chart = CreateChart( "chart-container", color, false );
    if ( UpdateChartData( mass ) && display_chart ) chart.render( );
...
function OnGainMass( me, other )
{
    ...
    else if ( !other.name && mass <= 1444 && ( mass >= 1369 || ( other.x == other.ox && other.y == other.oy ) ) )
    {
        ...
        { /*don't count own ejections, again not perfect*/
            stats.w.num++;
            stats.w.mass += mass;
        }
    }
    else
    {
        ...
        if ( stats.gains[ key ] == undefined ) stats.gains[ key ] = {
            num: 0,
            mass: 0
        };
        stats.gains[ key ].num++;
...
function OnLoseMass( me, other )
{
    ...
    var key = other.name + ':' + other.color;
    if ( stats.losses[ key ] == undefined ) stats.losses[ key ] = {
        num: 0,
        mass: 0
    };;
...
function DrawStats( game_over )
{
    ...
    list = jQuery( '<ol>' );
    if ( AppendTopN( 5, 'gains', list ) ) jQuery( '#gainArea' ).append( list );
    else jQuery( '#gainArea' ).append( '<ul><li style="font-size: 12px; ">You have not eaten anybody</li></ul>' );
    jQuery( '#lossArea' ).append( '<b>Top Losses</b>' );
    list = jQuery( '<ol>' );
    if ( AppendTopN( 5, 'losses', list ) ) jQuery( '#lossArea' ).append( list );
    else jQuery( '#lossArea' ).append( '<ul><li style="font-size: 12px; ">Nobody has eaten you</li></ul>' );
